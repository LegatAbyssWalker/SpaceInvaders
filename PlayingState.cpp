...
void PlayingState::handleKeyboardInputs(sf::Keyboard::Key key, bool isPressed) {
	...
	if (key == sf::Keyboard::R) {
		iBullet.setBulletPos(sf::Vector2<float>(screenWidth / 2, screenHeight / 2));
	}
}
...
void PlayingState::update() {
	...
	//Invader movements and Invader collision
	for (int x = 0; x < invaderVector.size(); x++) {
		...
		if (pBullet.collisionWithInvaders(invaderVector[x])) {
			...
			pBullet.setBulletPos({ 10000, 10000 });
	...
	for (int x = 0; x < playerVector.size(); x++) {
		if (iBullet.collisionWithPlayer(playerVector[x])) {
			iBullet.setBulletPos(sf::Vector2<float>(100000, 1000000));
			playerLives--;
			if (playerLives <= 0) { machine.run(machine.buildState<MainMenuState>(machine, window, true)); }
		}
	}
	...
	for (int x = 0; x < shieldVector.size(); x++) {
		...
		if (iBullet.collisionWithShield(shieldVector[x])) {
			...
			//Checking shield damage
			if (shieldVector[x]->shieldProtectionNum() <= 0) { shieldVector[x]->setShieldPos({ 10000, 100000 }); }
		}
		
		//Collision with player bullets
		if (pBullet.collisionWithShield(shieldVector[x])) {
			shieldVector[x]->shieldProtection(1);
			pBullet.setBulletPos(sf::Vector2<float>(100000, 100000));
			pBulletCount = 0;
	...
	for (int x = 0; x < ufoVector.size(); x++) {
		if (pBullet.collisionWithUFO(ufoVector[x])) {
			std::cout << "HIT\n";
			ufoVector[x]->setUFOPos({ 100000, 100000 });
			pBullet.setBulletPos({ 10000, 10000 });
			pBulletCount = 0;
			pBulletClock.restart();
		}
	...
	if (playerLives <= 0) {
		...
		playSound.setSound("res/soundsexplosion.wav", 35, false);
		if (deathTimer >= 2) { machine.run(machine.buildState<LoseMenuState>(machine, window, true)); playSound.stopMusic(); }
	}
}
